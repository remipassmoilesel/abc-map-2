...
public class Matcher {
	...
	public static Map<InterestPoint, InterestPoint> findMathes(
		...
		Map<InterestPoint, InterestPoint> matchedPoints = Matcher.findMatches(
				ipts1, ipts2);

		if (doReverseComparisonToo) {
			Map<InterestPoint, InterestPoint> matchedPointsReverse = Matcher
					.findMatches(ipts2, ipts1);
	...
	public static int countMatchesUsingHomography(
			Map<InterestPoint, InterestPoint> matches, ImagePlus imp1,
			int margin, float[][] h, ImagePlus imp2, float tolerance) {
		int count = 0;
	...
	static int translateCorners(Map<InterestPoint, InterestPoint> matches,
			Point2D[] src_corners, Point2D[] dst_corners) {
	...
	static double[] cvFindHomography(Point2Df[] objectPoints,
			Point2Df[] imagePoints, int method, double ransacReprojThreshold) {
