...
/**
 * Describe which geographic informations are displayed next a shape. This object should be serializable in a string composed of "1" and "0"
 *
 * @author remipassmoilesel
 */
public class GeoInfoMode {

    private static final CustomLogger logger = LogManager.getLogger(GeoInfoMode.class);
    ...
    public GeoInfoMode(String str) {
        ...
        if (str.length() < getArray().length) {
            ...
            logger.debug(new IllegalArgumentException("String to short"));
    ...
    public GeoInfoMode(boolean[] v) {
        ...
        if (v.length < getArray().length) {
            ...
            logger.debug(new IllegalArgumentException("Array to short"));
    ...
    /**
     * Return true if something have to be displayed
     *
     * @param mode
     * @return
     */
    public static boolean isInformationModeNotEmpty(GeoInfoMode mode) {
        ...
        // do not take care of non significative flag
