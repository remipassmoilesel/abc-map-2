...
import org.abcmap.core.importation.gpx.schema.gpxv1.Gpx;
import org.abcmap.core.importation.gpx.schema.gpxv1_1.*;
...
public class GpxParser {
        ...
        if (document instanceof Gpx) {
            ...
            for (Gpx.Rte route : gpxv1.getRte()) {
                ...
                for (Gpx.Rte.Rtept pt : route.getRtept()) {
                    ctr.getPoints().add(ptToGpxPoint(pt));
                }
                result.add(ctr);
            }
            ...
            for (Gpx.Trk track : gpxv1.getTrk()) {
                ...
                for (Gpx.Trk.Trkseg seg : track.getTrkseg()) {
                    for (Gpx.Trk.Trkseg.Trkpt pt : seg.getTrkpt()) {
                        ctr.getPoints().add(trkToGpxPoint(pt));
                    }
                }
                result.add(ctr);
            }
    ...
    private GpxPoint ptToGpxPoint(WptType pt) {
        String time = pt.getTime() != null ? pt.getTime().toString() : null;
        String comment = pt.getDesc() != null ? pt.getDesc().toString() : null;
        return new GpxPoint(pt.getLat(), pt.getLon(), pt.getEle(), time, comment);
    }

    private GpxPoint trkToGpxPoint(Gpx.Trk.Trkseg.Trkpt pt) {
        String time = pt.getTime() != null ? pt.getTime().toString() : null;
        String comment = pt.getDesc() != null ? pt.getDesc().toString() : null;
        return new GpxPoint(pt.getLat(), pt.getLon(), pt.getEle(), time, comment);
    }

    private GpxPoint ptToGpxPoint(Gpx.Rte.Rtept pt) {
        String time = pt.getTime() != null ? pt.getTime().toString() : null;
        String comment = pt.getDesc() != null ? pt.getDesc().toString() : null;
        return new GpxPoint(pt.getLat(), pt.getLon(), pt.getEle(), time, comment);
    }
